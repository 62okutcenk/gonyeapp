...
async def get_dashboard_stats(user: dict = Depends(get_current_user)):
    """
    Permission-based dashboard stats.
    - Finance data: requires 'projects.manage_finance' or is_admin
    - Workload data: requires 'users.view' or is_admin
    - Project counts: 'projects.view_all' sees all, others see only assigned projects
    - My tasks: Everyone sees their own
    """
    tenant_id = user["tenant_id"]
    user_id = user["id"]
    is_admin = user.get("is_admin", False)
    ...
    result = {
        ...
        "finance": None,  # Will be filled if user has permission
        "workload": None,  # Will be filled if user has permission
    }
    
    # ==================== PROJECT COUNTS ====================
    # If user has 'projects.view_all' permission, count all projects
    # Otherwise, count only projects where user is assigned
    ...
    if has_permission("projects.view_all"):
        ...
        if assigned_project_ids:
            ...
            # Status distribution for assigned projects
            status_counts = {}
            async for doc in db.projects.aggregate([
    ...
    # ==================== FINANCE DATA (Permission: projects.manage_finance) ====================
    if has_permission("projects.manage_finance"):
        # Calculate total revenue and collected from all areas
        pipeline = [
            {"$match": {"tenant_id": tenant_id}},
            {"$group": {
                "_id": None,
    ...
    # ==================== WORKLOAD DATA (Permission: users.view) ====================
    if has_permission("users.view"):
        ...
        result["workload"] = workload_data[:10]  # Top 10
...
async def global_search(q: str, user: dict = Depends(get_current_user)):
    """
    Global search across projects and customers.
    - Projects: If user has 'projects.view_all', search all. Otherwise, search only assigned.
    - Customers: Search all (CRM data is tenant-wide)
    """
    if not q or len(q) < 2:
    ...
    if not has_view_all:
        # Only search in assigned projects
        assigned_project_ids = await db.project_assignments.distinct(
            "project_id",
            {"user_id": user_id, "tenant_id": tenant_id}
        )
    ...
    # Search customers (always tenant-wide for CRM)
